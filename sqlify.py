...
def get_sql_data_type(cell):
    """Determine the data type of a given cell in a worksheet."""
    if cell.data_type == "n":  # Numeric
        return "INTEGER"
    elif cell.data_type == "s":  # String
        return "TEXT"
    elif cell.is_date:  # Date
        return "TEXT"
    else:
        raise NotImplementedError(
            "Datatype for cell {} is not supported".format(cell.coordinate)
        )
...
def get_values(worksheet, start_row=1):
    ...
    as the value will be converted into string object with the ISO 8601 format (YYYY-MM-DDTHH:MM:SS).
    """
    values = list()
    for row in tqdm(
        worksheet.iter_rows(min_row=2),
        desc=f"Importing '{worksheet.title}'",
        unit="row",
    ):
        db_row = list()
        for cell in row:
            if cell.is_date:
                db_row.append(
                    cell.value.strftime("%Y-%m-%dT%H:%M:%S")
...
def create_table(conn, cursor, table_name, columns):
    ...
    column_declarations = ",".join(
        ["{} {}".format(column[0], column[1]) for column in columns]
    )
    create_stmt = "CREATE TABLE IF NOT EXISTS {}({})".format(
        table_name, column_declarations
    )
...
def insert_values(conn, cursor, table_name, rows):
    ...
    insert_stmt = "INSERT INTO {}({}) VALUES ({})".format(
        table_name, column_names, ",".join(["?" for x in range(0, len(values[0]))])
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="A simple python script to create a sqlite database from a xlsx spreadsheet file."
    )
    parser.add_argument(
        "--db",
        type=str,
        default="",
        help="Path to the SQLite database. If no path is provided, a new database named [file] will be created.",
    )
    ...
    db_name = args.db or ".".join(
        [os.path.splitext(os.path.basename(args.file))[0], "sqlite3"]
    )

    conn = sqlite3.connect(db_name)
    cursor = conn.cursor()

    for worksheet in tqdm(
        workbook.worksheets, desc="Processing worksheets", unit="worksheet"
    ):
